...
{
 Process p;
 char nps[10],pids[10],buts[10],prios[10];
 int np,totaltime=0;
 do{
 ...
 while(i<np){
scanf("%s %s %s",pids,buts,prios);
if(isNum(pids)&&isNum(buts)&&isNum(prios)&&atoi(pids)>=0&&atoi(buts)>=0&&atoi(prios)>=0){
p.pid=atoi(pids);
p.but=atoi(buts);
p.priority=atoi(prios);
enqueue(p,&q);
i++;
 }
...
float sum=0;
printf("-----------------------\n");
printf("PID\t|Prio\t|Bur\t|WT\n");
printf("-----------------------\n");

while(!emptyqueue(&q)){

                dequeue(&p,&q);
                      printf("P%d\t|%d\t|%d\t%d\n",p.pid,p.priority,p.but,wt[i]);
